...
- Terminal scroll rework: the phone now owns the primary-screen viewport. Scrolling no longer moves the Mac's viewport, scrollback prefetch rebuilds keep your scroll position instead of snapping to the bottom, sustained scrolling pages deeper history (up to 2400 rows), and deep history now streams in bounded chunks so a huge snapshot can no longer stall the render pipeline and wipe scrollback mid-scroll. Alt-screen TUIs (vim/less/htop) still forward the wheel to the Mac.
